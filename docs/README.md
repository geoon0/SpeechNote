...
- **각 Phase가 끝나면 메인 브랜치로 머지하고 "시연 가능한 상태"를 만든다.** 다음 Phase는 그 위에서 분기. → 한쪽이 막혀도 직전 안정 상태로 발표·중간 점검 가능.
- **의존성은 단방향**: 민건영의 Service가 `TranscriptResult`를 반환 → 정의영이 그것을 받아 처리. 정의영이 민건영 코드를 고칠 일은 거의 없어야 함(반대만 OK).
- **Phase 1을 빨리 닫는 것이 학점 방어선.** 여기까지 안 닫히면 이후 Phase는 의미가 떨어지므로, 막히면 둘이 함께 페어 프로그래밍으로 강제로 닫는다.
- **가장 큰 리스크는 Phase 4의 시스템 음성 캡처(WASAPI loopback).** Phase 3 종료 시점에 한 번 "기술 점검" 마일스톤을 두고, 풀리지 않으면 "Windows + 마이크 단독"으로 후퇴할지 결정한다.
- **Phase 0의 `TranscriptResult` 합의가 가장 중요한 회의.** 이 객체 모양이 바뀌면 두 사람 코드 모두 갈아엎어야 하므로, 합의 후에는 변경 시 반드시 서로 통보.