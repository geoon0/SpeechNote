...
| 영역 | 담당 요구사항 | 내용 |
| --- | --- | --- |
| 입력·오디오·API | F-01 ~ F-05, F-13, F-14 | 실시간 녹음·장치 선택, 파일 업로드, 커스텀 STT/LLM API 통신, 설정 관리, 비동기·진행 표시, 요약·키워드 추출 |
| UI·DB·관리 | F-06 ~ F-12, F-15 ~ F-20 | 로그인·DB 연동, 히스토리 패널·검색·정렬, 내보내기, 원문 편집, 미니 플레이어, 에러 로깅, Two-Split UI |

- 영역 간 인터페이스는 공통 도메인 객체 `TranscriptResult`로 연결 — 의존성이 단방향이라 영역별 독립 개발이 수월함.
- **진행 방식**: 기능별 브랜치·커밋 컨벤션을 지키고, GitHub Issues로 요구사항(F-01~20)을 추적.
